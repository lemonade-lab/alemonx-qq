...
func TestCollectStatusNotInstalled(t *testing.T) {
	if platform := napcatPlatform(); platform != nil && (platform.Key == "darwin-external" || platform.Key == "windows-external") {
		t.Skip("desktop NapCat is download-only and intentionally has no runtime status")
	}
...
func TestNapCatStatusKeepsOneBotAccountsSeparate(t *testing.T) {
	if platform := napcatPlatform(); platform != nil && (platform.Key == "darwin-external" || platform.Key == "windows-external") {
		t.Skip("desktop NapCat is download-only and never reads OneBot configuration")
	}
	original := userConfigDir
	base := t.TempDir()
